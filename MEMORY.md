...
4. Chroma uses `GoogleGenerativeAiEmbeddingFunction` with `models/gemini-embedding-001` for document embeddings. The collection uses HNSW cosine distance.
5. Retrieval embeds queries with the Gemini `RETRIEVAL_QUERY` task type, then calls Chroma similarity search.
6. A LangChain `ChatPromptTemplate` injects retrieved context and citation labels into a grounded prompt.
7. Groq's `openai/gpt-oss-120b` answers both the RAG and no-RAG comparison paths.
...
- The index cell is idempotent. Set `REBUILD_INDEX = True` only when source PDFs or chunking settings change.
